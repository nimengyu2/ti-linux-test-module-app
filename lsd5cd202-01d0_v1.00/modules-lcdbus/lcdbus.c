...
#define DEVICE_NAME "buslcd" //设备名(/dev/lcd) 
...
void __iomem *lcd_write_cmd_addr;
void __iomem *lcd_write_data_addr;
...
{
	writeb(data, lcd_write_data_addr);
	//fn_delay_us(2);
}
...
{ 
	...
	lcd_write_cmd_addr = ((volatile unsigned long)ioremap(0x60000000,1));
	lcd_write_data_addr = ((volatile unsigned long)ioremap(0x60000008,1));
