...
void fn_lcd_init(void)
{
	...
	//fn_lcd_write_cmd(0x8E);			//12:RAM control,
	...
	fn_lcd_write_cmd(0xc4);			//enable FLT and FLB, 18:SET LCD MAPPING CONTROL
...
{ 
   ...
   switch (cmd)
   {
       case LCD_INIT:
       fn_lcd_init();
	   printk("lcd160160:lcd init\n");
       break;
       case LCD_WRITE_CMD:
       fn_lcd_write_cmd(arg&0xff);
		 //printk("lcd160160:write\n");
       break;
       case LCD_WRITE_DATA:
       fn_lcd_write_data(arg&0xff);
		 //printk("lcd160160:write data\n");
       break;
       default:
       printk("lcd160160: cmd error\n");
       return -EINVAL; 
   } 
...
{ 
    ...
    int i;  
    int status;
	write_cmd_addr = 	((volatile unsigned long)ioremap(0x60000000,1));
	write_data_addr = 	((volatile unsigned long)ioremap(0x60000004,1));
