...
//      }
     ...
     * 5. Flow Control
     */
    if((cgi_table[16-1].flag == 1) &&
       (cgi_table[17-1].flag == 1) &&
       (cgi_table[18-1].flag == 1) &&
       (cgi_table[19-1].flag == 1) &&
       (cgi_table[20-1].flag == 1)
       )
    {
        /*
         * 参数完备，可以存储
         */
        memset(file_s2wifi_cfg_buf[S2WIFI_CFG_VBAUDRATE], 0,
               sizeof(file_s2wifi_cfg_buf[S2WIFI_CFG_VBAUDRATE]));
        memset(file_s2wifi_cfg_buf[S2WIFI_CFG_VDATASIZE], 0,
               sizeof(file_s2wifi_cfg_buf[S2WIFI_CFG_VBAUDRATE]));
        memset(file_s2wifi_cfg_buf[S2WIFI_CFG_VPARITY], 0,
               sizeof(file_s2wifi_cfg_buf[S2WIFI_CFG_VBAUDRATE]));
        memset(file_s2wifi_cfg_buf[S2WIFI_CFG_VSTOP], 0,
               sizeof(file_s2wifi_cfg_buf[S2WIFI_CFG_VBAUDRATE]));
        memset(file_s2wifi_cfg_buf[S2WIFI_CFG_VBAUDRATE], 0,
               sizeof(file_s2wifi_cfg_buf[S2WIFI_CFG_VFLOW]));

        sprintf(file_s2wifi_cfg_buf[S2WIFI_CFG_VBAUDRATE], "vbaudrate=%s\n",
                cgi_table[16-1].buf);
        sprintf(file_s2wifi_cfg_buf[S2WIFI_CFG_VDATASIZE], "vdatasize=%s\n",
                cgi_table[17-1].buf);
        sprintf(file_s2wifi_cfg_buf[S2WIFI_CFG_VPARITY], "vparity=%s\n",
                cgi_table[18-1].buf);
        sprintf(file_s2wifi_cfg_buf[S2WIFI_CFG_VSTOP], "vstop=%s\n",
                cgi_table[19-1].buf);
        sprintf(file_s2wifi_cfg_buf[S2WIFI_CFG_VFLOW], "vflow=%s\n",
                cgi_table[20-1].buf);
