...
int tcp_client_init (void)
{
    ...
    else
    {
        ...
        {
            debug_out("socket create success,tcp_client_socket_fd = %d\n",
    ...
    // 服务器tcp端口和IP地址设定
    tcp_client_addr.sin_port=htons(s2wifi_cfg.vsocketrp);
    tcp_client_addr.sin_addr.s_addr = inet_addr(s2wifi_cfg.vsocketrip);
    ...
    if (res < 0)
    {
        ...
        if(errno != EINPROGRESS)
        {
            #if (TCP_CLIENT_DEBUG >= 1)
            {
                debug_out("socket connect  error\n");
            }
            #endif
            return -1;
        }
    ...
    #if (TCP_CLIENT_DEBUG >= 1)
    {
        debug_out("now connectting to remote IP is %s,remote port is %d\n",
                        s2wifi_cfg.vsocketrip,s2wifi_cfg.vsocketrp);
        debug_out("waitting for socket connect to server\n");
    }
    #endif
    return 0;
}
...
int tcp_client_connecting (void)
{
            ...
            &tcp_client_wset, NULL,&tcp_client_tval)) <= 0)
    {
        #if (TCP_CLIENT_DEBUG >= 3)
        {
            ...
            {
                info_cnt = 0;
                debug_out("no link ,is connecting  to server\n");
            }
        }
        #endif
        return -1;
    ...
    else
    {
        ...
        if (error)
        {
            #if (TCP_CLIENT_DEBUG >= 3)
            {
                ...
                // Error in connection() 113 - No route to host
                // Segmentation fault
                //debug_out("Error in connection() %d - %s\n", error, strerror(error));
                debug_out("Error in connection() %s",strerror(error));
            }
            #endif
            return -2;
        }
...
int tcp_client_read_data (unsigned char *buf,int len)
{
    ...
    if(tcp_client_socket_fd < 0)
    {
        #if (TCP_CLIENT_DEBUG >= 1)
        {
            debug_out("tcp_client_socket_fd below zero\n");
        }
        #endif
        // 读取错误
        return -2;
    ...
    if( n <= 0 )//rset没有使用过，不用重新置为sockfd
    {
        ...
        #if (TCP_CLIENT_DEBUG >= 3)
        {
            //debug_out("tcp_client no data to read\n");
        }
        #endif
        // 这里表示没有数据
        return -1;
...
int tcp_client_write (unsigned char* buf,int len)
{
    ...
    if ( (n = send(tcp_client_socket_fd, buf, len,0) ) ==-1 )
    {
        #if (TCP_CLIENT_DEBUG >= 1)
        {
            debug_out("tcp_client write data error\n");
        }
        #endif
        close(tcp_client_socket_fd);
        // 读取错误
...
int tcp_client_close ()
{
    if(tcp_client_socket_fd > 0)
    {
        ...
        #if (TCP_CLIENT_DEBUG >= 1)
        {
            debug_out("tcp_client close socket\n");
        }
        #endif
    }
}
