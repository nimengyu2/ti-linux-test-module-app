...
int wifi_process (void)
{
    ...
    s2wifi_config_wifi->wifi_process_init_ok = 1;
 ...
 fn_led_task();
      ...
      // connect_err_cnt += M_WIFI_PROCESS_USLEEP_CNT_MS;
       ...
       // 接受到关闭信号，此时关闭tcp 客户端
            ...
            tcp_read_num = tcp_server_read_data(pu8_tcp_read_buf,sizeof(pu8_tcp_read_buf));
            if(tcp_read_num  == 0)
            {
            ...
            {
                RingBufRead(&st_uart_to_wifi_data,pu8_tcp_write_buf,wifi_data_num);
                tcp_server_write(pu8_tcp_write_buf,wifi_data_num);
            }
