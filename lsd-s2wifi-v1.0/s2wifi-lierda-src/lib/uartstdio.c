...
//
// Porting form TI's Cortex M3 SW for s2wifi module by toby <toby.chs@gmail.com>
// @2011.09.23
//
//*****************************************************************************
...
UARTPrimeTransmit(unsigned long ulBase)
{
    ...
    if(!TX_BUFFER_EMPTY)
    {
        ...
        /*+*/ while(!TX_BUFFER_EMPTY)
        {
            ...
            if(g_bDisableEcho == false)
            {
                /*+*/ serial_send_char(g_pcUARTTxBuffer[g_ulUARTTxReadIndex]);
            }
...
/*-*/ //
/*-*/ //    //
/*-*/ //    // We are configured for buffered output so enable the master interr
/*-*/ //    // for this UART and the receive interrupts.  We don't actually enab
/*-*/ //    // transmit interrupt in the UART itself until some data has been pl
...
//*****************************************************************************
int
UARTwrite_noecho(const char *pcBuf, unsigned long ulLen)
{
#ifdef UART_BUFFERED
    unsigned int uIdx;
...
//*****************************************************************************
#include <at.h>
static char g_cmdline_last_history[CMDLINE_HISTORY_CNT][CMDLINE_BUF_SIZE];
static unsigned long g_last_count[CMDLINE_HISTORY_CNT] = {0};
static unsigned char g_cmdline_current_history[CMDLINE_BUF_SIZE] = {0};
static unsigned long g_cmdline_current_count = 0;
static unsigned long g_cmdline_start = 0; // 最久的历史命令
static unsigned long g_cmdline_end = 0; // 最近的历史命令
static unsigned long g_cmdline_current = 0; // 但前指向的用户命令
...
UARTgets(char *pcBuf, unsigned long ulLen)
{
    ...
    static char bLastWas_1b = 0;
    static char bLastWas_5b = 0;
    static char bLastCMD = 0;

    g_cmdline_current_count = 0;
    ...
    while(1)
    {
        ...
        /*+*/ serial_read_char(&cChar);
#if 0//DEBUG_UART_STDIO
        printf("[%s:%s:%d] cChar = %x\n",__FILE__,__FUNCTION__,__LINE__,
               cChar);
#endif
#if 0//DEBUG_UART_STDIO
        printf("[%s:%s:%d] cChar = %s\n",__FILE__,__FUNCTION__,__LINE__,
               pcBuf);
        printf("bLastWas_1b=%d, \n", bLastWas_1b);
        printf("bLastWas_5b=%d, \n", bLastWas_5b);
        printf("last_count=%d, \n", last_count);
        printf("ulCount=%d, \n", ulCount);
#endif
        //
        ...
        if(cChar == '\b')
        {
            ...
            if(ulCount)
            {
                ...
                //
                UARTwrite_noecho("\b \b", 3);

                //
                // Decrement the number of characters in the buffer.
                //
                ulCount--;
                g_cmdline_current_count--;
            }
        ...
        if(((cChar == 0x41) || (cChar == 0x44)) && bLastWas_5b)
        {
            bLastWas_1b = 0;
            bLastWas_5b = 0;
            // 替换先前的字符串
            ...
            if(bLastCMD == 1) // 纠正历史命令行索引值
            {
                g_cmdline_current = g_cmdline_end;
            }

            if(g_cmdline_current == g_cmdline_start)
            {
                // 不处理;
                bLastCMD = 0;
                strncpy(pcBuf, g_cmdline_last_history[g_cmdline_current],
                        g_last_count[g_cmdline_current]);
                UARTwrite_noecho(pcBuf, g_last_count[g_cmdline_current]);
                ulCount = g_last_count[g_cmdline_current];
#if DEBUG_UART_STDIO
            printf("[%s:%s:%d] cChar = %s\n",__FILE__,__FUNCTION__,__LINE__,
                   pcBuf);
            printf("bLastWas_1b=%d, \n", bLastWas_1b);
            printf("bLastWas_5b=%d, \n", bLastWas_5b);
            printf("g_last_count[g_cmdline_current]=%d, \n",
               g_last_count[g_cmdline_current]);
            printf("ulCount=%d, \n", ulCount);
            printf("g_cmdline_current=%d\n", g_cmdline_current);
            printf("g_cmdline_start=%d\n", g_cmdline_start);
            printf("g_cmdline_end=%d\n", g_cmdline_end);
            for(i=0; i<CMDLINE_HISTORY_CNT; i++)
            {
                printf("history[%d]=%s\n", i, g_cmdline_last_history[i]);
            }
#endif
                continue;
            }
            ...
            {
                bLastCMD = 0;
                if(g_cmdline_current == 0)
                {
                    g_cmdline_current = CMDLINE_HISTORY_CNT-1;
                }
                else
                {
                    g_cmdline_current = (g_cmdline_current - 1)%
                        CMDLINE_HISTORY_CNT;
                }
                strncpy(pcBuf, g_cmdline_last_history[g_cmdline_current],
                        g_last_count[g_cmdline_current]);
        ...
        else if(((cChar == 0x42) || (cChar == 0x43)) && bLastWas_5b)
        {
            bLastWas_1b = 0;
            bLastWas_5b = 0;
            // 替换先前的字符串
            ...
            g_cmdline_current = (g_cmdline_current + 1)%CMDLINE_HISTORY_CNT;
            if(g_cmdline_current == g_cmdline_end)
            {
                if(g_cmdline_current == 0)
                {
                    g_cmdline_current = CMDLINE_HISTORY_CNT-1;
                ...
                // 不处理
                bLastCMD = 1; // 记录本次为最后一次命令
                strncpy(pcBuf, g_cmdline_current_history,
                        g_cmdline_current_count);
                UARTwrite_noecho(pcBuf, g_cmdline_current_count);
                ulCount = g_cmdline_current_count;
#if DEBUG_UART_STDIO
            printf("[%s:%s:%d] cChar = %s\n",__FILE__,__FUNCTION__,__LINE__,
                   pcBuf);
            printf("bLastWas_1b=%d, \n", bLastWas_1b);
        ...
        if(ulCount < ulLen)
        {
            ...
            //
            pcBuf[ulCount] = cChar;
            g_cmdline_current_history[ulCount] = cChar;
    ...
    {
        // 记录上次的历史命令
        strncpy(g_cmdline_last_history[g_cmdline_end], pcBuf, ulCount+1);
        g_last_count[g_cmdline_end] = ulCount;

        // 索引偏移
        if(g_cmdline_end >= g_cmdline_start)
        {
            tmp = g_cmdline_end - g_cmdline_start + 1;
        }
        if(g_cmdline_end < g_cmdline_start)
        {
            tmp = CMDLINE_HISTORY_CNT - g_cmdline_start + g_cmdline_end + 1;
        }
        if(tmp == CMDLINE_HISTORY_CNT)
        {
            g_cmdline_start = (g_cmdline_start + 1) % CMDLINE_HISTORY_CNT;
        }
        g_cmdline_end = (g_cmdline_end + 1) % CMDLINE_HISTORY_CNT;
        g_cmdline_current = g_cmdline_end;
    }

    // 当前用户输入的记录缓存清空
    g_cmdline_current_count = 0;
    memset(g_cmdline_current_history, 0, sizeof(g_cmdline_current_history));
...
UARTEchoSet(bool_t bEnable)
{
    g_bDisableEcho = !bEnable;
}
/*-*/ //#endif
bool_t UARTEchoGet(void)
{
    return(g_bDisableEcho);
}

void uart_echo_set(bool_t bEnable)
{
    g_bDisableAtq = !bEnable;
}

bool_t uart_echo_get(void)
{
    return(g_bDisableAtq);
}
