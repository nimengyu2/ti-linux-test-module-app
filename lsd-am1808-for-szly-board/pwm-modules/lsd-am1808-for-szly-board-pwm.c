...
static unsigned char g_u8_pwm_on = 0;
static unsigned long g_u32_pwm_output_freq = 0;
static unsigned long g_u32_pwm_output_high_level_div = 2;
...
int fn_pwm_init(void)
{
	...
	// 比较值
	if(g_u32_pwm_output_high_level_div == 0)
	{
		g_u32_pwm_output_high_level_div = 2;
	}
	u16_tmp = u32_period/g_u32_pwm_output_high_level_div;
...
{ 
   	...
   	switch (cmd)
   	{
			...
			g_u32_pwm_output_high_level_div = 2;
			printk("pwm on,freq=%d,div=%d\n",(int)g_u32_pwm_output_freq,(int)g_u32_pwm_output_high_level_div);
       		fn_pwm_init();
      		break;
      		case SET_PWM_OFF:
			printk("pwm off now\n");
       		fn_pwm_exit();
       		break; 	
			case SET_PWM_HIGH_LEVEL_DIV :
			if(g_u8_pwm_on== 1)
			{
				g_u32_pwm_output_high_level_div = arg;
				printk("pwm on,freq=%d,div=%d\n",(int)g_u32_pwm_output_freq,(int)g_u32_pwm_output_high_level_div);
       			fn_pwm_init();
			}
			else
...
{ 
		...
		int status;		
		// 配置这个引脚为为EPWM0B功能
    	status = davinci_cfg_reg(DA850_EPWM0B);
    	if (status < 0) {
